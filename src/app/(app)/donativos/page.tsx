...
export default function DonationsPage() {
  ...
  const { register, handleSubmit, reset, control, setValue, formState: { errors } } = useForm<DonationInput>({ resolver: zodResolver(donationSchema), defaultValues: donationDefaults });
  const selectedMethod = useWatch({ control, name: "metodo_pago" });
  ...
  return <>
    ...
    <div className="alert alert-info"><HandCoins size={19} /><span><strong>Prototipo local.</strong> Los movimientos se guardan únicamente en este navegador hasta definir</span></div>
    ...
    {showForm && <form className="card donation-form" onSubmit={handleSubmit(submit)} noValidate>
      ...
      <div className="form-grid card-body">
        ...
        {(selectedMethod === "transferencia" || selectedMethod === "deposito") && <div className="field"><label className="required" htmlFor="donation-reference">Folio de {selectedMethod === "deposito" ? "depósito" : "transferencia"}</label><input id="donation-reference" {...register("referencia")} placeholder={`Escribe el folio de ${selectedMethod === "deposito" ? "depósito" : "transferencia"}`} />{errors.referencia && <span className="field-error">{errors.referencia.message}</span>}</div>}
